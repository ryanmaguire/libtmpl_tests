 ...
 ******************************************************************************/
#ifndef TMPL_TESTS_DAT2_TO_DAT2_VS_ARRAY_UNIT_TEST_H
#define TMPL_TESTS_DAT2_TO_DAT2_VS_ARRAY_UNIT_TEST_H
 ...
 *      ctype:                                                                *
 *          The external library type.                                        *
 *      ftype:                                                                *
 *          The float type (double, etc.).                                    *
 *      g:                                                                    *
 *          The generator for the external library type from two numbers.     *
 *      f0:                                                                   *
 ...
 *          The input array.                                                  *
 ******************************************************************************/
#define TMPL_DAT2_TO_DAT2_VS_ARRAY_UNIT_TEST(type, ctype, ftype, g, f0, f1, ia)\
int main(void)                                                                 \
{                                                                              \
    const type in[] = ia;                                                      \
    const size_t zero = TMPL_CAST(0, size_t);                                  \
    const size_t number_of_samples = TMPL_ARRAY_SIZE(in);                      \
    const ftype eps = TMPL_DEFAULT_TOLERANCE * TMPL_EPS(in[0].dat[0]);         \
    size_t n;                                                                  \
    for (n = zero; n < number_of_samples; ++n)                                 \
    {                                                                          \
        const type output = f0(in[n]);                                         \
        const ctype inval = g(in[n].dat[0], in[n].dat[1]);                     \
        const ctype outval = f1(inval);                                        \
